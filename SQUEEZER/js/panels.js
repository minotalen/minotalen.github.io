...
function renderPres(){
  /* the bar never pegs: under the req it reads banked/req, past it the
     fill walks this shard's segment, lo (the floor it sits on, clamped
     to the req gate) to next (the banked amount where the shard ticks +1) */
  const g=shardGain(),req=ascReq(),k=(1+.12*M('fortune'))*achS(),
        over=S.banked>=req,
        lo=over?Math.max(req,req*Math.pow(g/(ECO.SHARDS_BASE*k),2)):0,
        nt=over?req*Math.pow((g+1)/(ECO.SHARDS_BASE*k),2):req;
  /* the algebraic tick can land an ulp under the floor's real crossing:
     walk the integer until the live formula actually pays */
  let next=Math.ceil(nt);
  while(over&&Math.floor(ECO.SHARDS_BASE*Math.pow(next/req,.5)*k)<g+1)next++;
  const pc=Math.max(0,Math.min(100,(S.banked-lo)/(next-lo)*100));
  ...
  <div class="row pf pd" data-pk="asc" style="--p:${pc.toFixed(1)}%"><div class="b"><div class="nm">Shards waiting</div>
    <div class="ds">${over?`Next shard at ${fmt(next)} · ${fmt(next-S.banked)} banked to go`:`${fmt(S.banked)} / ${fmt(req)} to the first shard`}</div></div>
