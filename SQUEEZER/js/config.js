...
const STK = {
  ...
  tribute:{n:'Tribute',  t:2, pm:.7888, st:'whale',  d:'On draw: pays ×7 its value straight to score, kept on bust.',
           cur:(c,h)=>`now +${fmt(c.v*ECO.TRIBUTE_X*payMul(h))} a draw`},
  odds:   {n:'Odds',     t:1, pm:.6777, st:'shamrock',    d:'On draw: a free extra card comes, 50/50 it pays triple or nothing.'},
  snip:   {n:'Snip',     t:1, pm:.477, st:'peapod',  d:'On draw: a twin of its value waits in the discard till you score.'},
  mint:   {n:'Mint',     t:1, pm:.9555, st:'note',   d:'On bank: ×4 its value to score, on top of the payout.',
           cur:(c,h)=>`now +${fmt(cval(c)*ECO.MINT_X*payMul(h))} a bank`},
  brass:  {n:'Brass',    t:1, pm:.6444, st:'sunflower',  d:'On a table: each Brass pays +5 score per Brass card on table.',
           cur:(c,h)=>`now +${fmt(ECO.BRASS_SCORE*h.ids.length*h.ids.filter(id=>byId(id).stk==='brass').length)} a bank`},
  surge:  {n:'Surge',    t:2, pm:.5222, st:'wave', d:'On a table: +0.20 to the hand multiplier.',
  ...
  mirror: {n:'Mirror',   t:2, pm:.6444, st:'iceberg',  d:'Worth the highest value on the table.',
           cur:(c,h)=>`now pays ${fmt(stkBase(c,h))}`},
  ward:   {n:'Ward',     t:1, pm:.8555, st:'cactus', d:'On draw: a free extra card that cannot bust you.'},
  anchor: {n:'Anchor',   t:2, pm:1.2444, st:'anchor', d:'On draw: for the next 3 draws, a twin of this card slips back into the deck.',
           cur:(c,h)=>{const w=h&&h.run&&(h.run.anchWin||[]).find(x=>x.id===c.id);
             return w&&w.left>0?'guards '+w.left+' draw'+(w.left===1?'':'s'):''}},
  prime:  {n:'Prime',    t:3, pm:.9506, st:'coral',   d:'+2 to every other card on the table.',
  ...
  burn:   {n:'Burn',     t:2, pm:.7055, st:'flame',  d:'On draw: 2 cards of a random deck pair go to the discard.'},
  tell:   {n:'Tell',     t:2, pm:.8777, st:'periscope',    d:'Arm: the deck\'s top card flips face-up.'},
  cull:   {n:'Cull',     t:2, pm:1.3111, st:'funnel', d:'Arm: gain 1 ward for 3 draws, this card goes OUT.'},
  dividend:{n:'Dividend',t:2, pm:1.0361, st:'salmon',    d:'On bank: 25% chance the table scores again, a beat later.',
           cur:(c,h)=>`now 25% of ${fmt(handParts(h).total)}`},
  tab:    {n:'Tab',      t:3, pm:1.2049, st:'buoy', d:'On a table: +8% payout per card OUT.',
  ...
  bloom:  {n:'Bloom',    t:3, pm:1.0493, st:'tulip', d:'On bank: each Bloom flips a 50%. Hits pay +0.10× per card, per Bloom.',
           cur:(c,h)=>`now +${(ECO.BLOOM_PER*h.ids.filter(id=>byId(id).stk==='bloom').length*h.ids.length).toFixed(2)}× a hit`},
  kindle: {n:'Kindle',   t:3, pm:1.0814, st:'sun',  d:'On bank: each Kindle flips a 40%. Hits pay +0.40× per other card.',
           cur:(c,h)=>`now +${(ECO.KINDLE_PER*(h.ids.length-1)).toFixed(2)}× a hit`},
  jynx:   {n:'Jynx',     t:3, pm:1.1728, st:'hexnut', d:'The table pays +2% per risky card in the deck.',
           cur:(c,h)=>`now +${Math.round(ECO.JINX_PER*riskyIn(h)*100)}%`},
  reverb: {n:'Reverb',   t:2, pm:.5972, st:'ripples',   d:'On draw: the deck\'s top card goes to the discard; if it matches this card\'s value, gain 1 ward.'},
  siphon: {n:'Siphon',   t:2, pm:.4444, st:'whirl',   d:'On a bust: ×10 its value to score.',
           cur:(c,h)=>`now +${fmt(cval(c)*ECO.SIPHON_X*payMul(h))} on bust`},
  vanish: {n:'Vanish',   t:3, pm:.8765, st:'dusk',   d:'On bank: this card sits OUT till the next bust.'},
  ledger: {n:'Ledger',   t:3, pm:.9135, st:'journal', d:'Each card on the table with a twin OUT pays double.',
           cur:(c,h)=>`now ${h.ids.filter(id=>S.out.some(x=>cval(byId(x))===cval(byId(id)))).length} twins ×2`},
  rake:   {n:'Rake',     t:1, pm:.5222, st:'fern',   d:'On draw: +2 score per card OUT.',
           cur:(c,h)=>`now +${fmt(ECO.RAKE_PER*outCount()*payMul(h))} a draw`},
  stakes: {n:'Stakes',   t:3, pm:1.0246, st:'horseshoe',   d:'Arm: next 3 draws pay double premium.'},
  ...
  swap:   {n:'Swap',     t:1, pm:.5555, st:'leaves',   d:'Arm: becomes a random deck card\'s value, till it leaves the table.'},
  clip:   {n:'Clip',     t:2, pm:1.4277, st:'hook', d:'Arm: pick a table card, it loses 1 value till it leaves the table.'},
  ghost:  {n:'Ghost',    t:3, pm:1.1358, st:'pyrite', d:'Arm: pick a table card, it copies the table\'s lowest value till it leaves the table.'},
  patch:  {n:'Patch',    t:4, pm:2.4,  st:'chevron', d:'Arm: pick a table card, its value +1, for good.'},
  variety:{n:'Variety',  t:3, pm:1.232, st:'crown', d:'On a table: +10% mult per stickered card, itself included.',
  ...
  remnant:{n:'Remnant',  t:2, pm:.9499, st:'remnant', d:'On bank: if this card sits in the discard, its value pays too.',
  ...
  flinch: {n:'Flinch',   t:3, pm:1.0864, st:'flinch', d:'On draw: if you draw a card that matches this card\'s value, it goes OUT. One catch a run.'},
  offering:{n:'Offering',t:2, pm:1.1222, st:'offering', d:'Arm: this card leaves for OUT and the chain gains 1.'},
  recycle:{n:'Recycle',  t:3, pm:.9, st:'recycle', d:'On bank: the lowest-value card OUT returns to the deck.'},
  sub:    {n:'Sub',      t:2, pm:1.3416, st:'sub', d:'Arm: this card waits in the discard and a random card OUT takes its seat.'},
  exit:   {n:'Exit',     t:2, pm:1.0055, st:'exit', d:'On bank: 2% of the discard\'s total value pays to score.',
