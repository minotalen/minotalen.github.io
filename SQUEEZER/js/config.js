...
const STK = {
  gild:   {n:'Gilded',   t:3, pm:1.25, st:'sapphire',   d:'Worth ×2.75 its value.',
           cur:(c,h)=>`now +${fmt((ECO.GILD_X-1)*stkBase(c,h))}`},
  haste:  {n:'Haste',    t:1, pm:2.4688, st:'bolt',   d:'Draws come 25% faster.',
  ...
  surge:  {n:'Surge',    t:2, pm:.5222, st:'wave', d:'+0.20 mult.',
  ...
  prime:  {n:'Prime',    t:3, pm:.9506, st:'coral',   d:'+2 to every other table card.',
  ...
  purify: {n:'Purify',   t:5, pm:4.2, st:'crystal',   d:'Its value can never bust you.'},
  echo:   {n:'Echo',     t:4, pm:2.6, st:'megaphone',   d:'Tap to peek at the deck\'s top 3; twins go to the discard.'},
  bloom:  {n:'Bloom',    t:3, pm:1.0493, st:'tulip', d:'When banked, each Bloom rolls 1 in 2. Hits pay +0.10 mult per card, per Bloom.',
           cur:(c,h)=>`now +${(ECO.BLOOM_PER*h.ids.filter(id=>byId(id).stk==='bloom').length*h.ids.length).toFixed(2)}× a hit`},
  kindle: {n:'Kindle',   t:3, pm:1.0814, st:'sun',  d:'When banked, each Kindle rolls 2 in 5. Hits pay +0.40 mult per other card.',
  ...
  rake:   {n:'Rake',     t:1, pm:.5222, st:'fern',   d:'When drawn, pays +2 score per card in OUT.',
           cur:(c,h)=>`now +${fmt(ECO.RAKE_PER*outCount()*payMul(h))} a draw`},
  stakes: {n:'Stakes',   t:3, pm:1.0246, st:'horseshoe',   d:'Tap to double the risk reward for the next 3 draws.'},
  scrap:  {n:'Scrap',    t:3, pm:.9876, st:'pick',  d:'Tap to send the deck\'s top card OUT.'},
  relic:  {n:'Relic',    t:5, pm:6.2, st:'trophy',    d:'Gains +5% value at every bank.',
           cur:(c,h)=>`now +${Math.round((c.r||0)*ECO.RELIC_PER*100)}%`},
  purge:  {n:'Purge',    t:4, pm:2.8, st:'bin',    d:'When banked, the hand goes to the discard instead of home.'},
  encore: {n:'Encore',   t:5, pm:5.8, st:'curtain',   d:'When drawn, the deck and discard trade places.',
           cur:(c,h)=>S.disc&&S.disc.length?`now ${S.disc.length} in the discard`:''},
  float:  {n:'Float',    t:4, pm:2.2, st:'buoyring', d:'Tap to arm: past 70% on the gauge, the table pays flat and zeroes. No risk reward.'},
  defuse: {n:'Defuse',   t:4, pm:2.4, st:'plunger',  d:'Tap to cut the deck\'s top 2: one OUT, one to the discard. If neither would have busted, the table banks on the spot.'},
  swap:   {n:'Swap',     t:1, pm:.5555, st:'leaves',   d:'Tap to become a random deck card\'s value, till it leaves.'},
  clip:   {n:'Clip',     t:2, pm:1.4277, st:'hook', d:'Tap to pick a table card: it loses 1 value till it leaves.'},
  ghost:  {n:'Ghost',    t:3, pm:1.1358, st:'pyrite', d:'Tap to pick a table card: it copies the table\'s lowest value till it leaves.'},
  patch:  {n:'Patch',    t:4, pm:2.4,  st:'chevron', d:'Tap to pick a table card: it permanently gains +1 value.'},
  ...
  remnant:{n:'Remnant',  t:2, pm:.9499, st:'remnant', d:'While in the discard, pays its value each bank.',
           cur:(c,h)=>`now +${fmt(cval(c)*payMul(h))} a bank`},
  layaway:{n:'Layaway', t:3, pm:.8, st:'layaway', d:'+5% payout per card in the discard.',
           cur:(c,h)=>`now +${Math.round(ECO.LAY_PER*(S.disc?S.disc.length:0)*100)}%`},
  guardian:{n:'Guardian',t:4, pm:1.5625, st:'guardian', d:'While in OUT, any bust has a 1 in 3 chance to pay the table flat. Unique.'},
  ...
  whip:   {n:'Whip',    t:3, pm:1.35, st:'whip',  d:'When drawn or tapped, cuts the deck\'s first blank to the discard.'},
  strip:  {n:'Strip',   t:4, pm:2.2,  st:'strip', d:'Tap to send the table\'s lowest card to the discard for 1 ward.'},
  squeeze:{n:'Squeeze', t:5, pm:5.6,  st:'squeeze', d:'Tap to peek at 5 deck cards: this card becomes the sum of the risky ones, till it leaves.'},
  ...
  windfall:{n:'Windfall',t:4, pm:2.4, st:'windfall', d:'When drawn, 1 in 6 chance to double a random unstickered card on table.'},
  redline:{n:'Redline', t:4, pm:2.3,  st:'redline', d:'When drawn, lights a 3-draw fuse: past 70% at the burn, the standing score pays flat.'},
  tempo:  {n:'Tempo',   t:3, pm:1.1,  st:'tempo',  d:'When drawn, gains +1 value per chain link, till it leaves.',
           cur:(c,h)=>`now +${(h&&h.chain)||0} a draw`},
  barter: {n:'Barter',  t:3, pm:1.15, st:'barter', d:'Tap to pick a table card: it goes to the discard, a random OUT card lands. A twin still busts.'},
  recast: {n:'Recast',  t:4, pm:2.5,  st:'recast', d:'Tap to pick 1 of 3 from the discard: it lands, this card goes OUT. A twin still busts.'}
};
...
const STKTRIG = {
  ...
  bail:'tap', draft:'tap, keep 1 of 2', engrave:'tap, pick a table card',
  echo:'tap, scan the top 3', twin:'tap',
  anchor:'when drawn', reverb:'when drawn', ward:'when drawn', fallout:'when banked',
  odds:'when drawn',
  tribute:'when drawn', rake:'when drawn', snip:'when drawn', burn:'when drawn',
  mint:'when banked', dividend:'when banked, 1 in 4', vanish:'when banked', purge:'when banked',
  encore:'when drawn',
  bloom:'rolls on bank', kindle:'rolls on bank',
  relic:'every bank', siphon:'when its table busts',
  haste:'standing', beacon:'standing',
  remnant:'while in the discard', guardian:'while in OUT', flinch:'a drawn twin, once per run',
  offering:'tap', recycle:'when banked', sub:'tap',
  exit:'when banked',
  squeeze:'tap, scan 5', whip:'when drawn or tapped', strip:'tap',
  windfall:'when drawn, 1 in 6', redline:'when drawn, a 3-draw fuse',
  tempo:'when drawn', barter:'tap', recast:'tap, pick from the discard',
};
...
const COND = {
  ...
  ripped:  {n:'Ripped',    d:'When drawn, pays 1 ward, then deals 2 more cards.'}
};
const LINGO=[
  ...
  ['The gauge','Risky cards ÷ deck size. It shows the danger and sets the risk reward.'],
  ['Risk reward','The gauge prices every bank: half pay at 0% risk, face value at 50%, ×1.5 at 100%.'],
  ['Mult',"The hand multiplier: a table's total pays ×mult, and more cards on the table raise it."],
  ['Effect pays','Tribute, Rake, Siphon, Mint, Remnant and Exit pay their amount through every multiplier the table pays: value, hand mult, chain, risk reward, the works.'],
  ...
  ['On bank roll','Bloom, Kindle and Dividend gamble on the bank: every copy rolls its own chance, hits pay, misses sit. A Dividend hit scores the table again.'],
];
...
const META = {
  ...
  daring: {n:'Daring',       d:l=>'+8% to the risk reward, forever'+tot(8,l),      max:10, c:l=>Math.ceil(5*Math.pow(1.45,l))},
