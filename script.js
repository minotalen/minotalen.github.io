...
class HourliesApp {
    ...
    constructor() {
        ...
        this.isArrowNavigation = false; // Track if we're navigating with arrow keys (no animation)
...
▶️ Youtube`;
    ...
    processBubblesForMerging() {
        ...
        if (this.emojiMergeMode === 'all') {
            ...
            Object.entries(emojiGroups).forEach(([emoji, group]) => {
                ...
                // Calculate new radius with logarithmic scaling for area loss during merge
                // Instead of pure area conservation, use logarithmic scaling to lose some total area
                // This makes merging more realistic - merged bubbles are smaller than pure area sum
                const idealRadius = Math.sqrt(totalArea / Math.PI); // Pure area conservation
                const logScalingFactor = 0.8 + (0.15 * Math.log(group.length + 1) / Math.log(10)); // Scale 0.85-0.95 based on group size
                const newRadius = idealRadius * logScalingFactor;
                const newSize = newRadius * 2;

                mergedBubbles.push({
                    emoji,
                    count: group.length,
                    size: Math.min(Math.max(newSize, 40), 200), // Larger size range: 40-200px
                    id: `merged-${emoji}`,
                    x: Math.random() * 90 + 5, // Use more of the full width
                    y: Math.random() * 90 + 5  // Use more of the full height
                });
            });
            ...
            // Calculate container dimensions and target fill
            const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
            let estimatedContainerArea, targetFillPercent;
            
            if (isMobile) {
                // Mobile container is now 50% of viewport height for more square aspect
                ...
                targetFillPercent = 45; // Increased target fill for better visual density
            } else {
                // Desktop: actual container size is min(550px, 80vh) width varies
                const desktopHeight = Math.min(550, window.innerHeight * 0.8);
                const desktopWidth = Math.min(window.innerWidth * 0.8, 600); // Estimate container width
                estimatedContainerArea = desktopHeight * desktopWidth;
                targetFillPercent = 60; // Target 60% fill on desktop
            }
            
            const currentFillPercent = (totalBubbleArea / estimatedContainerArea) * 100;
            
            let scaleFactor = 1;
            // Only scale down if bubbles are too large (exceed target fill)
            if (mergedBubbles.length > 0 && currentFillPercent > targetFillPercent) {
                scaleFactor = Math.sqrt(targetFillPercent / currentFillPercent);
            }
            
            // Apply scaling to all bubbles (only scale down, never up)
            ...
            if (isMobile) {
                ...
                const mobileWidth = Math.min(window.innerWidth * 0.76, 400); // Account for 12% side padding
                estimatedContainerArea = mobileHeight * mobileWidth;
                targetFillPercent = 45;
            } else {
                // Desktop: actual container size is min(550px, 80vh) width varies
                const desktopHeight = Math.min(550, window.innerHeight * 0.5);
                const desktopWidth = Math.min(window.innerWidth * 0.5, 600); // Estimate container width
                estimatedContainerArea = desktopHeight * desktopWidth;
                targetFillPercent = 60; // Target 40% fill on desktop
            }
            ...
            let scaleFactor = 1;
            // Only scale down if bubbles are too large (exceed target fill)
            if (individualBubbles.length > 0 && currentFillPercent > targetFillPercent) {
                scaleFactor = Math.sqrt(targetFillPercent / currentFillPercent);
            }
    ...
    navigateDay(direction) {
        ...
        if (mainContainer) {
            mainContainer.classList.add(direction > 0 ? 'exiting-left' : 'exiting-right');
        }
        
        // After exit completes, render and play enter animation
        setTimeout(() => {
            // Remove exit class
            if (mainContainer) {
                mainContainer.classList.remove('exiting-left', 'exiting-right');
            }
            ...
            // Store the entering direction for use during render
            this.enteringDirection = direction > 0 ? 'entering-right' : 'entering-left';
            ...
            setTimeout(() => {
                const content = document.querySelector('.scroll-container, .daily-container, .settings-container');
                if (content) {
                    content.classList.remove('entering-left', 'entering-right');
                }
                this.enteringDirection = null;
            }, 300);
        }, 150);
    }
    ...
    setView(view) {
        ...
        const viewOrder = { 'timeline': 0, 'daily': 1, 'settings': 2 };
        const goingForward = viewOrder[view] > viewOrder[this.view];
        
        // Play exit animation
        const mainContainer = document.getElementById('mainContainer');
        if (mainContainer) {
            mainContainer.classList.add(goingForward ? 'exiting-left' : 'exiting-right');
        }
        ...
        setTimeout(() => {
            ...
            // Store the entering direction for use during render
            this.enteringDirection = goingForward ? 'entering-right' : 'entering-left';
    ...
    updateViewButtons() {
        const dailyBtn = document.querySelector('.btn[onclick*="daily"]');
        const settingsBtn = document.querySelector('.btn[onclick*="settings"]');
        
        [dailyBtn, settingsBtn].forEach(btn => {
            if (btn) btn.classList.remove('active-view');
        });
        
        if (this.view === 'daily' && dailyBtn) {
            dailyBtn.classList.add('active-view');
        } else if (this.view === 'settings' && settingsBtn) {
            settingsBtn.classList.add('active-view');
        }
    }
    ...
    finishEditing() {
        ...
        // Always play closing animation
        let animationDelay = 0;
        
        // Only animate if NOT arrow navigation (arrow nav gets instant appearance, needs closing animation on next edit)
        if (!this.isArrowNavigation) {
            const picker = document.querySelector('.emoji-picker');
            const overlay = document.querySelector('.emoji-picker-overlay');
            if (picker) {
                picker.classList.remove('opening');
        ...
        // Wait for closing animation before rendering
        setTimeout(() => {
            this.renderHours();
        }, animationDelay);
    }
    ...
    toggleEmoji(emoji) {
        ...
        // Update entries without adding to undo stack (we'll do that on edit completion)
        this.updateEntriesWithoutUndo(newEntries);
        // Set flag to prevent opening animation during emoji toggle
    ...
    handleHourInputKeydown(event, hour) {
        if (event.key === 'Enter' && !event.shiftKey) {
            event.preventDefault();
            // If we're arrow navigating, ensure closing animation plays
            if (this.isArrowNavigation) {
                const picker = document.querySelector('.emoji-picker');
                const overlay = document.querySelector('.emoji-picker-overlay');
                if (picker) {
            ...
            if (cursorAtEnd) {
                ...
                // Flag that we're doing arrow navigation (no shrink/grow animation)
                this.isArrowNavigation = true;

                // Finish current edit before moving to next (no closing animation)
                this.finishEditing();
                ...
                // Update scroll to center smoothly
                setTimeout(() => {
                    const hourElement = document.querySelector(`[data-hour="${nextHour}"]`);
                    if (hourElement) {
                        hourElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    }
                    const picker = document.querySelector('.emoji-picker');
                    if (picker) {
                        picker.classList.add('arrow-transition');
                    }
                }, 10);
    ...
    startBubbleAnimation() {
        ...
        // Mode state - only one can be active at a time
        let currentMode = 'gravity'; // 'gravity' | 'repulsion' | 'billiards'
        let isRepulsionLocked = false; // Mobile: lock repulsion mode until tap outside
        
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        
        // Physics constants
        const centerGravity = 0.022;
        const friction = 0.995;
        const gravityRestitution = 0.2; 
        const billiardsRestitution = 0.95;
        const repulsion = 0.2; // Increased from 0.2 to prevent overlaps
        const maxSpeed = 10;
        const minSpeed = 0;
        const gravityDampening = 0.7;
        
        // Shared attraction constants for same-type bubbles
        const attractionStrength = 1.2;
        const maxAttractionDistance = 600;
        const minDistance = 20; // Prevent division by very small numbers

        // Helper function to manage visual state with cleaner logic
        const updateContainerVisualState = () => {
            // Remove all mode classes first
            container.classList.remove('billiards-mode', 'repulsion-mode');
            
            // Apply class based on current mode
            if (currentMode === 'billiards') {
                container.classList.add('billiards-mode');
            } else if (currentMode === 'repulsion') {
                container.classList.add('repulsion-mode');
            }
            // gravity mode has no special class (default background)
        };

        // Initialize bubbles with improved overlap prevention
        bubbles.forEach((bubble) => {
            const size = parseFloat(bubble.style.width);
            const containerWidth = containerRect.width;
            const containerHeight = containerRect.height;
            
            // Increased padding and safety margin for merged bubbles
            const basePadding = size * 0.6; // 60% of bubble size as base padding
            const safetyMargin = size * 0.2; // Additional 20% safety margin
            const totalPadding = basePadding + safetyMargin;
            ...
            do {
                pixelX = totalPadding + Math.random() * (containerWidth - size - totalPadding * 2);
                pixelY = totalPadding + Math.random() * (containerHeight - size - totalPadding * 2);
                
                overlap = false;
                // Check against all existing bubbles with enhanced overlap detection
                bubbleStates.forEach((existingState) => {
                    ...
                    // Enhanced minimum distance calculation with safety buffer
                    const minDistance = (size + existingState.size) / 2 + Math.max(size, existingState.size) * 0.3;
                ...
                attempts++;
            } while (overlap && attempts < 150); // Increased max attempts for better placement
            
            // If we couldn't find a perfect spot after many attempts, use grid-based fallback
            if (overlap && attempts >= 150) {
                const gridCols = Math.max(2, Math.floor(containerWidth / (size * 1.5)));
                const gridRows = Math.max(2, Math.floor(containerHeight / (size * 1.5)));
                const gridIndex = bubbleStates.size;
                const col = gridIndex % gridCols;
                const row = Math.floor(gridIndex / gridCols) % gridRows;
                
                pixelX = (col + 0.5) * (containerWidth / gridCols) - size / 2;
                pixelY = (row + 0.5) * (containerHeight / gridRows) - size / 2;
                
                // Ensure grid position stays within bounds
                pixelX = Math.max(0, Math.min(pixelX, containerWidth - size));
                pixelY = Math.max(0, Math.min(pixelY, containerHeight - size));
            }
        ...
        const handleDragStart = (e, bubble) => {
            ...
            // On mobile, prevent drag if in locked repulsion mode
            if (isMobile && isRepulsionLocked) {
                return;
            }
            
            draggedBubble = bubble;
            currentMode = 'billiards';
            updateContainerVisualState();
            
            // Set touch/mouse position for repulsion calculations
            ...
            dragStartX = lastDragX = this.mouseX;
            dragStartY = lastDragY = this.mouseY;
        ...
        const handleDragMove = (e) => {
            ...
            // Update touch/mouse position for physics calculations
        ...
        const handleDragEnd = (e) => {
            ...
            // Transition to appropriate mode after drag ends
            if (isMobile) {
                // On mobile, go to gravity mode after drag (no repulsion flash)
                currentMode = 'gravity';
                this.mouseX = null;
                this.mouseY = null;
            } else {
                // On desktop, stay in billiards mode after drag
                currentMode = 'billiards';
            }
        ...
        const handleBlur = () => {
            if (!isMobile) {
                // Desktop: normal blur behavior
                currentMode = 'gravity';
                updateContainerVisualState();
            }
            // On mobile, blur doesn't change mode due to locking behavior
        };

        // Add click/tap outside container to exit billiards mode and unlock repulsion
        const handleOutsideClick = (e) => {
            // Check if click is outside the bubbles container
            if (!container.contains(e.target)) {
                if (isMobile) {
                    // Mobile: unlock repulsion and return to gravity
                    isRepulsionLocked = false;
                    currentMode = 'gravity';
                    this.mouseX = null;
                    this.mouseY = null;
                } else {
                    // Desktop: exit billiards mode
                    if (currentMode === 'billiards') {
                        currentMode = 'gravity';
                    }
                }
        ...
        const animate = (currentTime) => {
            ...
            bubbleStates.forEach((state, bubble) => {
                ...
                if (currentMode === 'gravity' || currentMode === 'repulsion') {
                    // Same-type attraction in touch merge mode (when not dragging or in billiards)
                    if (this.emojiMergeMode === 'touch') {
                        const currentEmoji = bubble.dataset.emoji;
                        const bubbleCenterX = state.x + state.size / 2;
                        const bubbleCenterY = state.y + state.size / 2;
                        
                        // Find and attract to nearest same-type bubble
                        let nearestSameType = null;
                        let nearestDistance = Infinity;
                        
                        bubbleStates.forEach((otherState, otherBubble) => {
                            if (otherBubble === bubble || otherBubble === draggedBubble) return;
                            if (otherBubble.dataset.emoji !== currentEmoji) return;
                            
                            const otherCenterX = otherState.x + otherState.size / 2;
                            const otherCenterY = otherState.y + otherState.size / 2;
                            const distance = Math.sqrt(
                                Math.pow(otherCenterX - bubbleCenterX, 2) + 
                                Math.pow(otherCenterY - bubbleCenterY, 2)
                            );
                            
                            if (distance < nearestDistance) {
                                nearestDistance = distance;
                                nearestSameType = { x: otherCenterX, y: otherCenterY, distance };
                            }
                        });
                        
                        // Apply attraction to nearest same-type bubble with inverse distance relationship
                        if (nearestSameType && nearestSameType.distance > 0) {
                            if (nearestSameType.distance < maxAttractionDistance) {
                                const dx = nearestSameType.x - bubbleCenterX;
                                const dy = nearestSameType.y - bubbleCenterY;
                                const dirX = dx / nearestSameType.distance;
                                const dirY = dy / nearestSameType.distance;
                                
                                // Inverse proportional force: stronger when closer (F ∝ 1/distance)
                                const effectiveDistance = Math.max(nearestSameType.distance, minDistance);
                                const force = attractionStrength / effectiveDistance;
                    ...
                    // Center gravity/repulsion (not in billiards mode)
                    const bubbleCenterX = state.x + state.size / 2;
                    const bubbleCenterY = state.y + state.size / 2;
                    const containerRadius = Math.min(containerRect.width, containerRect.height) / 2;
                    
                    // Default to container center if mouse is not set
                    const centerX = this.mouseX || containerRect.width / 2;
                    const centerY = this.mouseY || containerRect.height / 2;
                    ...
                    if (distToCenter > 0) {
                        ...
                        if (currentMode === 'repulsion') {
                            // Stronger repulsion when closer to mouse/touch
                            forceStrength = -centerGravity * 10 * (1 - Math.pow(normalizedDist, 0.8));
                        } else {
                            // Normal center attraction (gravity mode)
                ...
                } // End of gravity/repulsion when not in billiards mode

                // Apply friction
                state.vx *= friction;
                state.vy *= friction;

                // Apply minimal movement dampening (kill tiny movements) - only in gravity mode
                if (currentMode === 'gravity') {
                    const dampening = 0.007; // flat dampening amount
                    if (Math.abs(state.vx) < dampening) state.vx = 0;
                    if (Math.abs(state.vy) < dampening) state.vy = 0;
                    if (state.vx > 0) state.vx = Math.max(0, state.vx - dampening * 0.5);
                    if (state.vx < 0) state.vx = Math.min(0, state.vx + dampening * 0.5);
                    if (state.vy > 0) state.vy = Math.max(0, state.vy - dampening * 0.5);
                    if (state.vy < 0) state.vy = Math.min(0, state.vy + dampening * 0.5);
                }
                ...
                const currentRestitution = currentMode === 'billiards' ? billiardsRestitution : gravityRestitution;
                ...
                for (let j = i + 1; j < bubbleArray.length; j++) {
                    ...
                    if (distance < minDist) {
                        ...
                        if (this.emojiMergeMode === 'touch') {
                            ...
                            if (emoji1 === emoji2) {
                                // Determine which bubble is larger (will be the absorber)
                                const size1 = state1.size;
                                const size2 = state2.size;
                                
                                let absorber, absorbed, absorberState, absorbedState;
                                if (size1 >= size2) {
                                    absorber = bubble1;
                                    absorbed = bubble2;
                                    absorberState = state1;
                                    absorbedState = state2;
                                } else {
                                    absorber = bubble2;
                                    absorbed = bubble1;
                                    absorberState = state2;
                                    absorbedState = state1;
                                }
                                
                                // Calculate target size preserving total area
                                ...
                                const targetSize = Math.min(Math.max(newRadius * 2, 30), 150);
                                
                                // Calculate centers for momentum blending
                                const absorberCenterX = absorberState.x + absorberState.size / 2;
                                const absorberCenterY = absorberState.y + absorberState.size / 2;
                                const absorbedCenterX = absorbedState.x + absorbedState.size / 2;
                                const absorbedCenterY = absorbedState.y + absorbedState.size / 2;
                                
                                // Smooth velocity blending based on mass
                                const mass1 = size1 * size1; // Mass proportional to area
                                const mass2 = size2 * size2;
                                const totalMass = mass1 + mass2;
                                
                                // Weighted velocity average based on mass
                                const blendedVx = (absorberState.vx * (absorberState.size * absorberState.size) + 
                                                    absorbedState.vx * (absorbedState.size * absorbedState.size)) / 
                                                    (absorberState.size * absorberState.size + absorbedState.size * absorbedState.size);
                                const blendedVy = (absorberState.vy * (absorberState.size * absorberState.size) + 
                                                    absorbedState.vy * (absorbedState.size * absorbedState.size)) / 
                                                    (absorberState.size * absorberState.size + absorbedState.size * absorbedState.size);
                                
                                // Apply blended velocity to absorber
                                absorberState.vx = blendedVx;
                                absorberState.vy = blendedVy;
                                
                                // Complete merge immediately
                                absorberState.size = targetSize;
                                
                                // Update bubble count
                                const count1 = parseInt(bubble1.dataset.count || '1');
                                const count2 = parseInt(bubble2.dataset.count || '1');
                                absorber.dataset.count = count1 + count2;
                                
                                // Update visual properties with smooth transition for size change
                                absorber.style.transition = 'width 0.15s ease-out, height 0.15s ease-out, font-size 0.15s ease-out';
                                absorber.style.width = `${targetSize}px`;
                                absorber.style.height = `${targetSize}px`;
                                absorber.style.fontSize = `${targetSize * 0.85}px`;
                                
                                // Clean up transition after animation
                                setTimeout(() => {
                                    absorber.style.transition = '';
                                }, 150);
                                
                                // Remove absorbed bubble immediately
                                bubblesToRemove.add(absorbed);
                        ...
                        const repulsionForce = repulsion * 1; // Stronger force to maintain separation
        ...
        // Focus/blur event listeners for the container
        container.addEventListener('focus', (e) => {
            if (!isMobile && currentMode !== 'billiards') {
                // Only set mouse coordinates if they weren't already set by click
                if (this.mouseX === null || this.mouseX === undefined) {
                    // Fallback to container center if no mouse position available
                    this.mouseX = containerRect.width / 2;
                    this.mouseY = containerRect.height / 2;
                }
                currentMode = 'repulsion';
                updateContainerVisualState();
            }
        });
        ...
        container.addEventListener('blur', () => {
            if (!isMobile) {
                currentMode = 'gravity';
                updateContainerVisualState();
                // Clear mouse position when container loses focus on desktop
                this.mouseX = null;
                this.mouseY = null;
            }
        });
        ...
        container.addEventListener('mouseenter', (e) => {
            if (!isMobile && currentMode === 'repulsion') {
                const rect = container.getBoundingClientRect();
        ...
        // Click to focus the container (desktop only)
        container.addEventListener('click', (e) => {
            if (!isMobile && currentMode !== 'billiards') {
                // Set mouse coordinates immediately on click before focusing
                const rect = container.getBoundingClientRect();
                this.mouseX = e.clientX - rect.left;
                this.mouseY = e.clientY - rect.top;
                container.focus();
        ...
        // Touch to enter repulsion mode (mobile only)
        container.addEventListener('touchstart', (e) => {
            if (isMobile && e.target === container) {
                // Enter locked repulsion mode on mobile
                currentMode = 'repulsion';
                isRepulsionLocked = true;
                const rect = container.getBoundingClientRect();
        ...
        // Touch move to update repulsion center (mobile only)
        container.addEventListener('touchmove', (e) => {
            if (isMobile && currentMode === 'repulsion' && e.target === container) {
                const rect = container.getBoundingClientRect();
                this.mouseX = e.touches[0].clientX - rect.left;
                this.mouseY = e.touches[0].clientY - rect.top;
                e.preventDefault(); // Prevent scrolling when in repulsion mode
            }
        }, { passive: false });
    ...
    renderTimeline() {
        const enterClass = this.enteringDirection ? ` ${this.enteringDirection}` : '';
        document.getElementById('mainContainer').innerHTML = `
            <div class="scroll-container${enterClass}">
                <div class="date-nav">
            ...
            <div class="hours-container">
                ${this.hours.map(hour => {
                    ...
                    return `
                        <div class="hour-row">
                            ${isEditing ? `
                                <div class="emoji-picker below active ${!this.isArrowNavigation ? 'opening' : ''}">
                ...
                ${this.editingHour !== null ? `<div class="emoji-picker-overlay active ${!this.isArrowNavigation ? 'opening' : ''}" onclick="app.finishEditing()"></div>` : ''}
            </div>
        `;
    ...
    renderSettings() {
        const enterClass = this.enteringDirection ? ` ${this.enteringDirection}` : '';
        document.getElementById('mainContainer').innerHTML = `
            <div class="settings-container${enterClass}">
    ...
    renderDaily() {
        ...
        document.getElementById('mainContainer').innerHTML = `
            <div class="daily-container${enterClass}" onclick="app.resetWipeConfirmation()">
                ...
                <div class="bubbles-container" tabindex="0">
                    ${(() => {
